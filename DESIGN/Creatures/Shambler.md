...
placed as corridor/room encounters — sometimes an ambush timed to an item pickup (the Utility Room
screwdriver), sometimes just a room the player wasn't expecting one in, once as a named/story-tied
reanimation (Fennimore, in the courtyard), and once, deliberately, as a **two-Shambler pincer**
(East Wing Housekeeping Closet, Scene 43) that removes the player's usual option to just
retreat — rather than an open avoid-or-engage choice, since the hotel's tight interiors don't
really allow for that. Once the game opens up outdoors (Chapter 2 onward), that changes: used to
gate/populate open spaces (e.g. the four shamblers inside Memorial Park on first arrival) and to
give the player real agency (avoid vs. engage, as established explicitly in Chapter 2's street
crossing).

## Encounter Progression

- First **directly depicted on-screen**: throughout the Ravenwood Hotel during the East/West Wing
  power-restoration loop and the courtyard endgame (Chapter 1) — ten total, spread across both
  wings and the courtyard: seven one-on-one placements (second-floor east hallway, Dining Hall,
  Utility Room, Lounge/Recreation, Main Bar, Boiler Room, West Wing Service Corridor), a
  **two-Shambler pincer ambush** in the East Wing Housekeeping Closet (Scene 43) — the chapter's
  first no-clean-retreat combat moment, sprung from Rooms 114 and 116 on a deliberate second pass
  through a hallway the player already thought was clear — and a named exception, **Fennimore's
  reanimation** in the courtyard (Scene 44). Deliberately not in every room, so placement stays
  unpredictable rather than a checklist. See
...
- [`Scripts/Chapter_1_One_Night_Only.md`](../Scripts/Chapter_1_One_Night_Only.md), Scenes 30, 31, 32, 37 (×3), 39, 43 (×2,
  pincer ambush), and 44 (Fennimore, reanimated) — ten total corridor/room encounters throughout
  the hotel and courtyard.
- Referenced by name (not shown) in [`Scripts/Chapter_1_One_Night_Only.md`](../Scripts/Chapter_1_One_Night_Only.md), Scene 45 (Caretaker
  boss design notes).
